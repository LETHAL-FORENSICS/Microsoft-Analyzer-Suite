-- Title: Set-InboxRule
-- Description: Modify an existing Inbox Rule in a mailbox (Suspicious Operation)
-- Id: 34267727-bbfe-4b3e-af69-be2b9d8b1bd4
...
SELECT
  ...
  CASE 
    ...
    ELSE Merged
  END AS IPAddress,
  list_extract(list_filter(auditData.Parameters, lambda x: x.Name = 'Identity'), 1).Value AS Name,
...
WHERE Operation = 'Set-InboxRule' OR Operation = 'set-InboxRule'
